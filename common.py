...
class WriteType(Enum):
    ...
    DATAFRAMES = "dataframes" # List of names of all dataframes listed for PandasAI
    ...
    PANDASAIERR = "pandasai_error" # The exact error encountered by PandasAI when trying to execute generated scripts
    CODE = "code" # Last Python code generated by PandasAI (on error)
...
class Common:
    ...
    def pd_ai_is_expected_type(self, expected_type:str, response, lenient:bool=True):
        
        if isinstance(response,str):
            if r'^Unfortunately, I was not able to answer your question'.find(response) > -1:
                return False
        ...
        if lenient:
            return isinstance(response,(str,int,float,np.number))
    ...
    def get_response_type(self, expected_type: str, response) -> Tuple[str,bool]:
        
        if expected_type == "dataframe":
            if isinstance(response,(DataFrame,SmartDataframe)):
                # Expected df, got df
                return "dataframe", True
            # expected df, got something else
            return "dataframe", False
        
        if isinstance(response,(DataFrame,SmartDataframe)):
            # Expected string, plot or number, got df
            return "dataframe", False
        
        if isinstance(response,str):
            if re.search(r"^Unfortunately, I was not able to answer your question", response):
                # Expected something, got error
                return "error", False
            if bool(re.match(r"^(\/[\w.-]+)+(/[\w.-]+)*$|^[^\s/]+(/[\w.-]+)*$", response)):
                # Got plot
                if expected_type == "plot":
                    # Expected plot, got plot
                    return "plot", True
                if expected_type == "string":
                    # Expected string, got plot
                    return "plot", True
            # Expected something, got string
            return "string", True
        
        if isinstance(response,(int,float,np.number)):
            # Expected something, got number
            return "number", True
